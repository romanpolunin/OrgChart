...
namespace Staffer.OrgChart.Layout
{
    ...
    public static class LayoutAlgorithm
    {
        ...
        /// </summary>
        public static Rect ComputeBranchVisualBoundingRect([NotNull]Tree<int, Box> visualTree)
        {
            var result = visualTree.Roots[0].Element.Frame.Exterior;

            Tree<int, Box>.TreeNode.IterateParentFirst(visualTree.Roots[0], node =>
            {
                var box = node.Element;

                if (box.IsVisible && !box.IsAutoGenerated)
                {
            ...
            tree.IterateParentFirst(
                node =>
                    node.Element.IsVisible =
                        ...
                        node.ParentNode.Element.IsVisible && !node.ParentNode.Element.IsCollapsed);
        ...
        {
            if (!branchRoot.Element.IsVisible)
            {
                return;
            }
